...
Usage:
    python scripts/convert_pgdump_to_json.py src/planner/data/benchmarks/performance/integ-oct-29.sql
    # Creates: src/planner/data/benchmarks/performance/benchmarks_GuideLLM.json

    python scripts/convert_pgdump_to_json.py src/planner/data/benchmarks/performance/integ-oct-29.sql -o src/planner/data/custom_output.json
    # Creates: src/planner/data/custom_output.json

The output JSON file will have the same format as benchmarks_BLIS.json and can be
loaded using the standard JSON loader.

Requirements:
    - Docker running with planner-postgres container (make db-start)
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to output JSON file (default: src/planner/data/benchmarks/performance/benchmarks_GuideLLM.json)",
    )
    parser.add_argument(
        "--pretty",
        action="store_true",
