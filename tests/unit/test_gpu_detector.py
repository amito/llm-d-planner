...
class TestGPUProductMap:
    ...
    def test_all_map_values_are_canonical(self):
        from neuralnav.cluster.gpu_detector import GPU_PRODUCT_MAP
        from neuralnav.shared.utils.gpu_normalizer import CANONICAL_GPUS
...
class TestDetectClusterGPUs:
    ...
    def setup_method(self):
        from neuralnav.cluster.gpu_detector import reset_gpu_cache

        reset_gpu_cache()

    @patch("neuralnav.cluster.gpu_detector._HAS_KUBERNETES", False)
    ...
    @patch("neuralnav.cluster.gpu_detector._HAS_KUBERNETES", True)
    @patch("neuralnav.cluster.gpu_detector._load_k8s_config")
    @patch("neuralnav.cluster.gpu_detector._list_nodes")
    ...
    @patch("neuralnav.cluster.gpu_detector._HAS_KUBERNETES", True)
    @patch("neuralnav.cluster.gpu_detector._load_k8s_config", side_effect=Exception("no config"))
    ...
    @patch("neuralnav.cluster.gpu_detector._HAS_KUBERNETES", True)
    @patch("neuralnav.cluster.gpu_detector._load_k8s_config")
    @patch("neuralnav.cluster.gpu_detector._list_nodes", side_effect=Exception("API error"))
